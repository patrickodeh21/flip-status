...
class ChecklistController extends Controller
{
    public function toggle(ChecklistToggleRequest $request, CleaningSession $session, Room $room, Task $task)
    {
        ...
        if ($request->has('force_checked')) {
            $nowChecked = $request->boolean('force_checked');
        } else {
    ...
    public function taskPhoto(Request $request, CleaningSession $session, Room $room, Task $task)
    {
        ...
        // If a photo was uploaded, process and store it
    ...
    /**
     * Delete a photo for a specific task (room-level).
     */
    public function deletePhoto(Request $request, CleaningSession $session, Room $room, Task $task, ChecklistItemPhoto $photo)
    {
        ...
        if (!$item || $photo->checklist_item_id !== $item->id) {
        ...
        // Auto-untick if no photos remain
        if (ChecklistItemPhoto::where('checklist_item_id', $item->id)->count() === 0) {
            $item->update(['checked' => false, 'checked_at' => null]);
        }
    ...
    /**
     * Delete a photo for a property-level task.
     */
    ...
    private function processAndStorePhoto($file, CleaningSession $session): array
    {
        ...
        // Persist to long-term storage when available (e.g., serverless deployments)
        if (class_exists(\App\Services\PersistentPhotoStorage::class)) {
            try {
                PersistentPhotoStorage::persistFromPublicDisk($storedPath);
            } catch (\Throwable $e) {
                Log::warning('ChecklistController: Unable to persist checklist photo', [
